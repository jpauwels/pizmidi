...
public:
    //==============================================================================
    JuceVSTWrapper (audioMasterCallback audioMaster, PizAudioProcessor* const filter_)
...
#define PIZ_FX_MAGIC 1
	...
	if (hostname=="REAPER") {
		filter->reaper = true;
		char ReaperFunctionName[] = "TimeMap2_timeToBeats";
		if (audioMaster(NULL,0xdeadbeef,0xdeadf00d,0,ReaperFunctionName,0.0))
			*(VstIntPtr *)&(filter->TimeMap2_timeToBeats) = (VstIntPtr)audioMaster(NULL,0xdeadbeef,0xdeadf00d,0,ReaperFunctionName,0.0);
		else filter->reaper = false;
		strcpy(ReaperFunctionName, "GetPlayPosition");
		if (audioMaster(NULL,0xdeadbeef,0xdeadf00d,0,ReaperFunctionName,0.0))
			*(VstIntPtr *)&(filter->GetPlayPosition) = (VstIntPtr)audioMaster(NULL,0xdeadbeef,0xdeadf00d,0,ReaperFunctionName,0.0);
		else filter->reaper = false;
		strcpy(ReaperFunctionName, "GetPlayPosition2");
		if (audioMaster(NULL,0xdeadbeef,0xdeadf00d,0,ReaperFunctionName,0.0))
			*(VstIntPtr *)&(filter->GetPlayPosition2) = (VstIntPtr)audioMaster(NULL,0xdeadbeef,0xdeadf00d,0,ReaperFunctionName,0.0);
		else filter->reaper = false;
		strcpy(ReaperFunctionName, "GetCursorPosition");
		if (audioMaster(NULL,0xdeadbeef,0xdeadf00d,0,ReaperFunctionName,0.0))
			*(VstIntPtr *)&(filter->GetCursorPosition) = (VstIntPtr)audioMaster(NULL,0xdeadbeef,0xdeadf00d,0,ReaperFunctionName,0.0);
		else filter->reaper = false;
		strcpy(ReaperFunctionName, "get_config_var");
		if (audioMaster(NULL,0xdeadbeef,0xdeadf00d,0,ReaperFunctionName,0.0))
			*(VstIntPtr *)&(filter->get_config_var) = (VstIntPtr)audioMaster(NULL,0xdeadbeef,0xdeadf00d,0,ReaperFunctionName,0.0);
		else filter->reaper = false;
	}
...
//====================================================
    ...
    bool getOutputProperties (VstInt32 index, VstPinProperties* properties)
    {
        ...
        if (filter == nullptr || index >= numOut)
            return false;
