...
{
    if (index == kChannel) return T("Channel");
    return T("param")+String(index);
}

const String MidiCurve::getParameterText (int index) 
{
    if (index==kChannel) {
        if (roundToInt(param[kChannel]*16.0f)==0) return String(T("Any"));
...
void MidiCurve::setCurrentProgramStateInformation (const void* data, int sizeInBytes)
{
    ...
    if (xmlState != 0)
    {
        ...
        if (xmlState->hasTagName (T("MYPLUGINSETTINGS")))
        {
			...
			lastUIWidth = programs[curProgram].lastUIHeight = xmlState->getIntAttribute (T("uiWidth"), lastUIWidth);
            lastUIHeight = programs[curProgram].lastUIWidth = xmlState->getIntAttribute (T("uiHeight"), lastUIHeight);
